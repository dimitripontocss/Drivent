...
export async function createOrUpdateEnrollmentWithAddress(params: CreateEnrollmentParams) {
  ...
  const alreadyExistEnrollment = await prisma.enrollment.findFirst({ where: { userId: params.userId } });

  if (alreadyExistEnrollment) {
    await prisma.address.deleteMany({ where: { enrollmentId: alreadyExistEnrollment.id } });
  }
